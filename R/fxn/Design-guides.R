...
fxn$Design_guides_to_model_transitions_with_BE3_and_ABE <- function(df,
  ...
  result <- 
    ...
    mutate(
      ...
      ref_minus   = fxn$dna_reverse_complement(ref_plus),
      alt_minus   = fxn$dna_reverse_complement(alt_plus),
      ...
      ref_frame    = fxn$reading_frame(POS),
      # Translate all three frames of the alternative sequence context
      frame1_plus  = fxn$dna_translate(str_sub(alt_plus,  1L, 57L)),
      frame2_plus  = fxn$dna_translate(str_sub(alt_plus,  2L, 58L)),
      frame3_plus  = fxn$dna_translate(str_sub(alt_plus,  3L, 59L)),
      frame1_minus = fxn$dna_translate(str_sub(alt_minus, 1L, 57L)),
      frame2_minus = fxn$dna_translate(str_sub(alt_minus, 2L, 58L)),
      frame3_minus = fxn$dna_translate(str_sub(alt_minus, 3L, 59L)),
  ...
  for (spacing in ABE_spacing) {
    ...
    result[[ABE_model]]  <- fxn$extract_guide(result$ref_plus, target_position = flanking + 1, base_edit = 'a', spacing = spacing, pam = ABE_PAM_pattern)
    result[[ABE_revert]] <- fxn$extract_guide(result$alt_plus, target_position = flanking + 1, base_edit = 'a', spacing = spacing, pam = ABE_PAM_pattern)
